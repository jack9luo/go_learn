...
// 使用 WaitGroup 来告知外部“该协程已完成”。
// 使用轻微的 Sleep 仅用于演示并发交错输出，非业务必需。
...
// 与 printOdds 同理，结束时通过 Done() 通知 WaitGroup。
...
// - 对每个任务记录开始时间并在结束后将结果写入带缓冲的通道；
// - 统一 Wait 后关闭通道，再收集所有结果。
func RunTasksConcurrently(tasks map[string]Task) []TaskResult {
	...
	for name, task := range tasks {
		waitGroup.Add(1)
		go func(taskName string, t Task) { // 匿名函数 - 立即执行
			defer waitGroup.Done()
			startedAt := time.Now()
			t()
